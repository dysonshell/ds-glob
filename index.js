...
var DSC = process.env.DS_COMPONENT_PREFIX || 'dsc';
DSC = DSC.replace(/\/+$/, '') + '/';

var dsGlob = exports = module.exports = function (pattern, cb) {
    if (pattern.indexOf(DSC) !== 0) {
        return cb(new Error('dsGlob() only finds you files starting with `${process.env.DS_COMPONENT_PREFIX}/`.'));
    }
    co(function *() {
        cb(null, _.uniq(
...
dsGlob.sync = function (pattern) {
    if (pattern.indexOf(DSC) !== 0) {
        throw new Error('dsGlob() only finds you files starting with `${process.env.DS_COMPONENT_PREFIX}/`.');
    }
    return _.uniq(
        glob.sync(pattern, {
            cwd: APP_ROOT
